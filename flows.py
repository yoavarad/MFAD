...
FLOWNET_INPUT_WIDTH = defaultdict(lambda: 1024, FLOWNET_INPUT_WIDTH)
FLOWNET_INPUT_HEIGHT = defaultdict(lambda: 640, FLOWNET_INPUT_HEIGHT)


def extracting_flows(dataset_name, root, train): #, start_index=0, end_index=-1):
    if train:
    ...
    for idx, (batch, _) in tqdm(enumerate(dataset_loader), total=len(dataset)):
        # idx = len(dataset) - idx - 1  # reverse order
        ...
        if os.path.isfile(os.path.join(video_of_path, cur_img_name + '.npy')):
            continue
        # else:
        #     print('Processing:', os.path.join(video_of_path, cur_img_name + '.npy'))
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--start_index", type=int, default=0, help='start index of the test dataset')
    # parser.add_argument("--end_index", type=int, default=-1, help='end index of the test dataset')
