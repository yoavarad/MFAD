...
def get_foreground(img, bboxes, patch_size):
    """
    Cropping the object area according to the bounding box, and resize to patch_size
    ...
    if len(img.shape) == 3:
        ...
        for i in range(len(bboxes)):
            x_min, x_max = np.int32(np.ceil(bboxes[i][0])), np.int32(np.ceil(bboxes[i][2]))
            y_min, y_max = np.int32(np.ceil(bboxes[i][1])), np.int32(np.ceil(bboxes[i][3]))
            cur_patch_set: np.ndarray = img[:, :, y_min:y_max, x_min:x_max]
            tmp_set = []
            if cur_patch_set.size == 0:
                continue
...
class VideoDataset(Dataset):
    def __init__(self, dataset_name, root="data/", train=True, sequence_length=0, mode="last",
    ...
    def __initialize(self):
        if self.train:
            self.root_frames = os.path.join(glob.escape(self.root), 'training', 'frames/*')
        else:
            self.root_frames = os.path.join(glob.escape(self.root), 'testing', 'frames/*')

        videos = glob.glob(self.root_frames)
        for i, video_path in enumerate(sorted(videos)):
            self.videos[i] = {}
            self.videos[i]['path'] = video_path
            self.videos[i]['frames'] = glob.glob(os.path.join(glob.escape(video_path), '*.jpg'))
    ...
    def get_gt(self):
        ...
        elif self.dataset_name == 'HMDB51_AD':
            self.all_gt = []
            for i, (_, video) in enumerate(self.videos.items()):
                if 'run_' in video['path'] or 'walk_' in video['path']:
        ...
        elif self.dataset_name == 'HMDB-Violence':
            self.all_gt = []
            for i, (_, video) in enumerate(self.videos.items()):
                normal = False
                for cat in ['walk', 'run', 'wave', 'turn', 'throw', 'sit', 'hug', 'climb', 'cartwheel']:
                    if f'{cat}_' in video['path']:
                        normal = True
                        break
                if normal:
                    gt = torch.zeros(video['length'])
    ...
    def get_sequence(self, index):
        ...
        if all_frames_same_video[0] != 0 and all_frames_same_video[
            -1] != 0:  # Extreme condition that is not likely to happen
    ...
    def __getitem__(self, index):
        # reverse index
        # index = self.num_of_frames - index - 1
...
class VideoDatasetWithFlows(Dataset):
    ...
    def __initialize(self):
        if self.train:
            self.root_frames = os.path.join(glob.escape(self.root), 'training', 'frames/*')
            self.root_frames_flows = os.path.join(glob.escape(self.root), 'training', 'flows/*')
        else:
            self.root_frames = os.path.join(glob.escape(self.root), 'testing', 'frames/*')
            self.root_frames_flows = os.path.join(glob.escape(self.root), 'testing', 'flows/*')

        videos = glob.glob(self.root_frames_flows)
        for i, video_path in enumerate(sorted(videos)):
            self.flows[i] = {}
            self.flows[i]['path'] = video_path
            self.flows[i]['frames'] = glob.glob(os.path.join(glob.escape(video_path), '*.npy'))
    ...
    def get_gt(self):
        if self.dataset_name == 'ped2':
            ...
            vid_list = os.listdir(f'{self.root}/testing/frames')
            vid_list.sort()
            self.all_gt = []
            for i, (_, video) in enumerate(self.videos.items()):
                if 'run_' in video['path'] or 'walk_' in video['path']:
